...
if (Uc_A_1>0&&Uc_B_1<0&&Uc_C_1<=0 )
    ...
    %整流端选择 S1 S5 以及 S1 S6
    Vdc_a=1*Uc_A_1+(-1)*Uc_B_1;%%S1 S5的状态下的Vdc
    Vdc_b=1*Uc_A_1+(-1)*Uc_C_1;%%S1 S6的状态下的Vdc
...
elseif Uc_A_1>0&&Uc_B_1>=0&&Uc_C_1<0
    ...
    %整流端选择 S2 S6 以及 S1 S6
    Vdc_a=1*Uc_A_1+(-1)*Uc_C_1;%%S1 S6的状态下的Vdc
    Vdc_b=1*Uc_B_1+(-1)*Uc_C_1;%%S2 S6的状态下的Vdc
...
elseif Uc_B_1>0&&Uc_A_1<=0&&Uc_C_1<0  
    ...
    %整流端选择 S2 S6 以及 S2 S4
    Vdc_a=1*Uc_B_1+(-1)*Uc_C_1;%%S2 S6的状态下的Vdc
    Vdc_b=1*Uc_B_1+(-1)*Uc_A_1;%%S2 S4的状态下的Vdc
...
elseif Uc_A_1<0&&Uc_B_1>0&&Uc_C_1>=0
    sita=pi+sita;
    %整流端选择 S2 S4 以及 S3 S4
    Vdc_a=1*Uc_B_1+(-1)*Uc_A_1;%%S2 S4的状态下的Vdc
    Vdc_b=1*Uc_C_1+(-1)*Uc_A_1;%%S3 S4的状态下的Vdc
%     da=(-1)*(Uc_B_1/Uc_A_1);db=(-1)*(Uc_C_1/Uc_A_1);
...
elseif Uc_C_1>0&&Uc_B_1<=0&&Uc_A_1<0
    sita=pi+sita;
    %整流端选择 S3 S4 以及 S3 S5
    Vdc_a=1*Uc_C_1+(-1)*Uc_A_1;%%S3 S4的状态下的Vdc
    Vdc_b=1*Uc_C_1+(-1)*Uc_B_1;%%S3 S5的状态下的Vdc
%     da=(-1)*(Uc_A_1/Uc_C_1);db=(-1)*(Uc_B_1/Uc_C_1);
   da=(sin(sita-7*pi/6)); db=sin(3*pi/2-(sita)); dc=1-da-db;
    ...
    %整流端选择 S3 S5 以及 S1 S5
    Vdc_a=1*Uc_C_1+(-1)*Uc_B_1;%%S3 S5的状态下的Vdc
    Vdc_b=1*Uc_A_1+(-1)*Uc_B_1;%%S1 S5的状态下的Vdc
%     da=(-1)*(Uc_C_1/Uc_B_1);db=(-1)*(Uc_A_1/Uc_B_1);
...
%%整流级也运用mpc选择的思想，将输入电压的扇区变化为（0，3/Π）%%
...
if(sector==1)%%为了保证直流环节为postive，可选择的整流级开关状态为（S1,S5),(S1,S6)以及(S1,S5),(S2,S6)
    Va=[1 0 0 0 1 0];Vb=[1 0 0 0 0 1];%%整流级选择15 16
    Vdc_a=Uc_A_1+(-1)*Uc_B_1;
    Vdc_b=Uc_A_1+(-1)*Uc_C_1;
    da=sin(sita);db=sin(pi/3-sita);
    Vdc1=da*Vdc_a+db*Vdc_b;
    ...
    g1=(ga*gb)/(ga+gb);
    Va=[1 0 0 0 1 0];Vb=[0 1 0 0 0 1];%%整流级选择15 26，为了将合成的电压矢量处于1扇区，此时的da必须大于等于db
    Vdc_a=Uc_A_1+(-1)*Uc_B_1;
    Vdc_b=Uc_B_1+(-1)*Uc_C_1;
    
elseif(sector==2)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S5),(S1,S5)以及(S3,S5),(S1,S6)
    Va=[0 0 1 0 1 0];Vb=[1 0 0 0 1 0];%%整流级选择35 15
    Vdc_a=Uc_C_1+(-1)*Uc_B_1;
    Vdc_b=Uc_A_1+(-1)*Uc_B_1;
    Va=[0 0 2 0 1 0];Vb=[1 0 0 0 0 1];%%整流级选择35 16，为了将合成的电压矢量处于2扇区，此时的da必须大于等于db
    Vdc_a=Uc_C_1+(-1)*Uc_B_1;
    Vdc_b=Uc_A_1+(-1)*Uc_C_1;
elseif(sector==3)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S4),(S3,S5)以及(S3,S4),(S1,S5)
    Va=[0 0 1 1 0 0];Vb=[0 0 1 0 1 0];%%整流级选择34 35
    Vdc_a=Uc_C_1+(-1)*Uc_A_1;
    Vdc_b=Uc_C_1+(-1)*Uc_B_1;
    Va=[0 0 1 1 0 0];Vb=[1 0 0 0 1 0];%%整流级选择34 15，为了将合成的电压矢量处于3扇区，此时的da必须大于等于db
    Vdc_a=Uc_C_1+(-1)*Uc_A_1;
    Vdc_b=Uc_A_1+(-1)*Uc_B_1;
elseif(sector==4)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S4),(S3,S4)以及(S2,S4),(S3,S5)
    Va=[0 1 0 1 0 0];Vb=[0 0 1 1 0 0];%%整流级选择24 34
    Vdc_a=Uc_B_1+(-1)*Uc_A_1;
    Vdc_b=Uc_C_1+(-1)*Uc_A_1;
    Va=[0 1 0 1 0 0];Vb=[0 0 1 0 1 0];%%整流级选择24 35，为了将合成的电压矢量处于4扇区，此时的da必须大于等于db
    Vdc_a=Uc_B_1+(-1)*Uc_A_1;
    Vdc_b=Uc_C_1+(-1)*Uc_B_1;
elseif(sector==5)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S6),(S2,S4)以及(S2,S6),(S3,S4)
    Va=[0 1 0 0 0 1];Vb=[0 1 0 1 0 0];%%整流级选择26 24
    Vdc_a=Uc_B_1+(-1)*Uc_C_1;
    Vdc_b=Uc_B_1+(-1)*Uc_A_1;
    Va=[0 1 0 0 0 1];Vb=[0 0 1 1 0 0];%%整流级选择26 34，为了将合成的电压矢量处于5扇区，此时的da必须大于等于db
    Vdc_a=Uc_B_1+(-1)*Uc_C_1;
    Vdc_b=Uc_C_1+(-1)*Uc_A_1;
elseif(sector==1)%%为了保证直流环节为postive，可选择的整流级开关状态为（S1,S6),(S2,S6)以及(S1,S6),(S2,S4)
    Va=[1 0 0 0 0 1];Vb=[0 1 0 0 0 1];%%整流级选择16 26
    Vdc_a=Uc_A_1+(-1)*Uc_C_1;
    Vdc_b=Uc_B_1+(-1)*Uc_C_1;
    Va=[1 0 0 0 0 1];Vb=[0 1 0 1 0 0];%%整流级选择16 24，为了将合成的电压矢量处于6扇区，此时的da必须大于等于db 
    Vdc_a=Uc_A_1+(-1)*Uc_C_1;
    Vdc_b=Uc_B_1+(-1)*Uc_A_1;
end

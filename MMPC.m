...
if(sector==1)%%为了保证直流环节为postive，可选择的整流级开关状态为（S1,S5),(S1,S6)以及(S1,S5),(S2,S6)
    ...
    da1=sin(pi/3-sita);db1=sin(sita);
    Vdc1=da1*Vdc_a+db1*Vdc_b;
    ...
    if(g1>=g2)
        ...
        Va1=Va2;Vb1=Vb2;da1=da2;db1=db2;Vdc1=Vdc2;
    end
    
    
elseif(sector==2&&Us_alphar>=0)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S5),(S1,S5)以及(S3,S5),(S1,S6)
    Va1=[0 0 1 0 1 0];Vb1=[1 0 0 0 1 0];%%整流级选择35 15
    Vdc_a=Uc_C_1+(-1)*Uc_B_1;
    Vdc_b=Uc_A_1+(-1)*Uc_B_1;
    da1=sin(2*pi/3-(sita));db1=sin(sita-pi/3);
    Vdc1=da1*Vdc_a+db1*Vdc_b;
    Ic_A_a=1*Idc;Ic_B_a=(-1)*Idc;Ic_C_a=0;%%%在整流级为100010时的Ic
    Ic_A_b=1*Idc;Ic_B_b=0;Ic_C_b=(-1)*Idc;%%%在整流级为100001时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
...
elseif(sector==3)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S4),(S3,S5)以及(S3,S4),(S1,S5)
    ...
    da1=sin(pi-(sita));db1=sin(sita-2*pi/3);
    Vdc1=da1*Vdc_a+db1*Vdc_b;
    Ic_A_a=1*Idc;Ic_B_a=(-1)*Idc;Ic_C_a=0;%%%在整流级为100010时的Ic
    Ic_A_b=1*Idc;Ic_B_b=0;Ic_C_b=(-1)*Idc;%%%在整流级为100001时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
...
elseif(sector==4)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S4),(S3,S4)以及(S2,S4),(S3,S5)
    ...
    da1=sin(4*pi/3-sita);db1=sin(sita-pi);
    Vdc1=da1*Vdc_a+db1*Vdc_b;
    Ic_A_a=1*Idc;Ic_B_a=(-1)*Idc;Ic_C_a=0;%%%在整流级为100010时的Ic
    Ic_A_b=1*Idc;Ic_B_b=0;Ic_C_b=(-1)*Idc;%%%在整流级为100001时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
...
elseif(sector==5&&Us_alphar<=0)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S6),(S2,S4)以及(S2,S6),(S3,S4)
    sita=pi+sita;
    Va1=[0 1 0 0 0 1];Vb1=[0 1 0 1 0 0];%%整流级选择26 24
    Vdc_a=Uc_B_1+(-1)*Uc_C_1;
    Vdc_b=Uc_B_1+(-1)*Uc_A_1;
    da1=sin(5*pi/3-sita);db1=sin(sita-4*pi/3);
    Vdc1=da1*Vdc_a+db1*Vdc_b;
    Ic_A_a=1*Idc;Ic_B_a=(-1)*Idc;Ic_C_a=0;%%%在整流级为100010时的Ic
    Ic_A_b=1*Idc;Ic_B_b=0;Ic_C_b=(-1)*Idc;%%%在整流级为100001时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
...
elseif(sector==6)%%为了保证直流环节为postive，可选择的整流级开关状态为（S1,S6),(S2,S6)以及(S1,S6),(S2,S4)
    ...
    Vdc_b=Uc_B_1+(-1)*Uc_C_1;
    da1=sin(2*pi-sita);db1=sin(sita-5*pi/3);
    Vdc1=da1*Vdc_a+db1*Vdc_b;
    Ic_A_a=1*Idc;Ic_B_a=(-1)*Idc;Ic_C_a=0;%%%在整流级为100010时的Ic
    Ic_A_b=1*Idc;Ic_B_b=0;Ic_C_b=(-1)*Idc;%%%在整流级为100001时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);

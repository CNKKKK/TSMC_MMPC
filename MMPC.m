...
if(sector==1)%%为了保证直流环节为postive，可选择的整流级开关状态为（S1,S5),(S1,S6)以及(S1,S5),(S2,S6)
    Va1=[1 0 0 0 1 0];Vb1=[1 0 0 0 0 1];%%整流级选择15 16
    Vdc_a=Uc_A_1+(-1)*Uc_B_1;
    Vdc_b=Uc_A_1+(-1)*Uc_C_1;
    da=sin(pi/3-sita);db=sin(sita);
    ...
    Va2=[1 0 0 0 1 0];Vb2=[0 1 0 0 0 1];%%整流级选择15 26，为了将合成的电压矢量处于1扇区，此时的da必须大于等于db
    Vdc_a=Uc_A_1+(-1)*Uc_B_1;
    Vdc_b=Uc_B_1+(-1)*Uc_C_1;
    da2=sin(pi/3-sita);db2=sita(2*pi/3-(pi/3-sita));
    Vdc2=da2*Vdc_a+db2*Vdc_b;
    Ic_A_a=1*Idc;Ic_B_a=(-1)*Idc;Ic_C_a=0;%%%在整流级为100010时的Ic
    Ic_A_b=0;Ic_B_b=1*Idc;Ic_C_b=(-1)*Idc;%%%在整流级为100001时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
    Ic_alphar_b=(2/3)* (Ic_A_b-(1/2)*Ic_B_b-(1/2)*Ic_C_b);      Ic_beta_b=(2/3)*((sqrt(3)/2)*Ic_B_b-(sqrt(3)/2)*Ic_C_b);
     Is_alphar_a_1=-0.0248*Uc_alphar_1+0.9817*Is_alphar+0.0248*Us_alphar+0.0059*Ic_alphar_a;
     Is_beta_a_1=-0.0248*Uc_beta_1+0.9817*Is_beta+0.0248*Us_beta+0.0059*Ic_beta_a;
...
elseif(sector==2&Uc_alphar_1>=0)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S5),(S1,S5)以及(S3,S5),(S1,S6)
    Va1=[0 0 1 0 1 0];Vb1=[1 0 0 0 1 0];%%整流级选择35 15
    Vdc_a=Uc_C_1+(-1)*Uc_B_1;
    Vdc_b=Uc_A_1+(-1)*Uc_B_1;
    da=sin(2*pi/3-(sita));db=sin(sita-pi/3);
    Vdc=da*Vdc_a+db*Vdc_b;
    ...
    Va2=[0 0 1 0 1 0];Vb2=[1 0 0 0 0 1];%%整流级选择35 16，为了将合成的电压矢量处于2扇区，此时的da必须大于等于db
    Vdc_a=Uc_C_1+(-1)*Uc_B_1;
    Vdc_b=Uc_A_1+(-1)*Uc_C_1;
    da2=sin(2*pi/3-(sita));db2=sin(2*pi/3-(2*pi/3-(sita)));
    Vdc2=da2*Vdc_a+db2*Vdc_b;
    Ic_A_a=0*Idc;Ic_B_a=(-1)*Idc;Ic_C_a=1*Idc;%%%在整流级为001010时的Ic
    ...
    if(g1>=g2)
        ...
        Va1=Va2;Vb1=Vb2;da1=da2;db1=db2;Vdc=Vdc2;
    end
...
elseif(sector==2&Uc_alphar_1<0)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S5),(S1,S5)以及(S3,S5),(S1,S6)
    ...
    Va2=[0 0 2 0 1 0];Vb2=[1 0 0 0 0 1];%%整流级选择35 16，为了将合成的电压矢量处于2扇区，此时的da必须大于等于db
    Vdc_a=Uc_C_1+(-1)*Uc_B_1;
    Vdc_b=Uc_A_1+(-1)*Uc_C_1;
    da2=sin(2*pi/3-(sita));db2=sin(2*pi/3-(2*pi/3-(sita)));
    Vdc2=da2*Vdc_a+db2*Vdc_b;
...
elseif(sector==3)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S4),(S3,S5)以及(S3,S4),(S1,S5)
    sita=pi+sita;
    Va1=[0 0 1 1 0 0];Vb1=[0 0 1 0 1 0];%%整流级选择34 35
    Vdc_a=Uc_C_1+(-1)*Uc_A_1;
    Vdc_b=Uc_C_1+(-1)*Uc_B_1;
    da=sin(pi-(sita));db=sin(sita-2*pi/3);
    Vdc=da*Vdc_a+db*Vdc_b;
    ...
    Va2=[0 0 1 1 0 0];Vb2=[1 0 0 0 1 0];%%整流级选择34 15，为了将合成的电压矢量处于3扇区，此时的da必须大于等于db
    Vdc_a=Uc_C_1+(-1)*Uc_A_1;
    Vdc_b=Uc_A_1+(-1)*Uc_B_1;
    da2=sin(pi-(sita));db2=sin(2*pi/3-(pi-(sita)));
    Vdc2=da2*Vdc_a+db2*Vdc_b;
    Ic_A_a=(-1)*Idc;Ic_B_a=(0)*Idc;Ic_C_a=1*Idc;%%%在整流级为001100时的Ic
    Ic_A_b=1*Idc;Ic_B_b=(-1)*Idc;Ic_C_b=(0)*Idc;%%%在整流级为100010时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
...
elseif(sector==4)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S4),(S3,S4)以及(S2,S4),(S3,S5)
    sita=pi+sita;
    Va1=[0 1 0 1 0 0];Vb1=[0 0 1 1 0 0];%%整流级选择24 34
    Vdc_a=Uc_B_1+(-1)*Uc_A_1;
    Vdc_b=Uc_C_1+(-1)*Uc_A_1;
    da=sin(4*pi/3-sita);db=sin(sita-pi);
    Vdc=da*Vdc_a+db*Vdc_b;
    ...
    Va2=[0 1 0 1 0 0];Vb2=[0 0 1 0 1 0];%%整流级选择24 35，为了将合成的电压矢量处于4扇区，此时的da必须大于等于db
    Vdc_a=Uc_B_1+(-1)*Uc_A_1;
    Vdc_b=Uc_C_1+(-1)*Uc_B_1;
    da2=sin(4*pi/3-sita);db2=sin(2*pi/3-(4*pi/3-sita));
    Vdc2=da2*Vdc_a+db2*Vdc_b;
    Ic_A_a=(-1)*Idc;Ic_B_a=(1)*Idc;Ic_C_a=0;%%%在整流级为010100时的Ic
    Ic_A_b=0*Idc;Ic_B_b=(-1)*Idc;Ic_C_b=(1)*Idc;%%%在整流级为001010时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
...
elseif(sector==5&Uc_alphar_1<=0)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S6),(S2,S4)以及(S2,S6),(S3,S4)
    sita=pi+sita;
    Va1=[0 1 0 0 0 1];Vb1=[0 1 0 1 0 0];%%整流级选择26 24
    Vdc_a=Uc_B_1+(-1)*Uc_C_1;
    Vdc_b=Uc_B_1+(-1)*Uc_A_1;
    da=sin(5*pi/3-sita);db=sin(sita-4*pi/3);
    Vdc=da*Vdc_a+db*Vdc_b;
    Ic_A_a=(-1)*Idc;Ic_B_a=(1)*Idc;Ic_C_a=0;%%%在整流级为010100时的Ic
    Ic_A_b=(-1)*Idc;Ic_B_b=0;Ic_C_b=(1)*Idc;%%%在整流级为001100时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
    ...
    Va2=[0 1 0 0 0 1];Vb2=[0 0 1 1 0 0];%%整流级选择26 34，为了将合成的电压矢量处于5扇区，此时的da必须大于等于db
    Vdc_a=Uc_B_1+(-1)*Uc_C_1;
    Vdc_b=Uc_C_1+(-1)*Uc_A_1;
    da2=sin(5*pi/3-sita);db2=sin(2*pi/3-(5*pi/3-sita));
    Vdc2=da2*Vdc_a+db2*Vdc_b;
    Ic_A_a=(-1)*Idc;Ic_B_a=(1)*Idc;Ic_C_a=0;%%%在整流级为010100时的Ic
    Ic_A_b=0*Idc;Ic_B_b=(-1)*Idc;Ic_C_b=(1)*Idc;%%%在整流级为001010时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
...
elseif(sector==5&Uc_alphar_1>0)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S6),(S2,S4)以及(S2,S6),(S3,S4)
    ...
    if(g1>=g2)
        ...
        Va1=Va2;Vb1=Vb2;da=da2;db=db2;Vdc1=Vdc2;
    end
    
elseif(sector==6)%%为了保证直流环节为postive，可选择的整流级开关状态为（S1,S6),(S2,S6)以及(S1,S6),(S2,S4)
    Va1=[1 0 0 0 0 1];Vb1=[0 1 0 0 0 1];%%整流级选择16 26
    Vdc_a=Uc_A_1+(-1)*Uc_C_1;
    Vdc_b=Uc_B_1+(-1)*Uc_C_1;
    da=sin(2*pi-sita);db=sin(sita-5*pi/3);
    Vdc=da*Vdc_a+db*Vdc_b;
    ...
    Va2=[1 0 0 0 0 1];Vb2=[0 1 0 1 0 0];%%整流级选择16 24，为了将合成的电压矢量处于6扇区，此时的da必须大于等于db 
    Vdc_a=Uc_A_1+(-1)*Uc_C_1;
    Vdc_b=Uc_B_1+(-1)*Uc_A_1;
    da2=sin(2*pi-sita);db2=sin(2*pi-(sita-5*pi/3));
    Vdc2=da2*Vdc_a+db2*Vdc_b;
    Ic_A_a=1*Idc;Ic_B_a=(0)*Idc;Ic_C_a=1*Idc;%%%在整流级为100001时的Ic

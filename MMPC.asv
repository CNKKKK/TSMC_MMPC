...
if(sector==1)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S6),(S1,S6)以及(S1,S5),(S2,S6)
    ...
    Va2=[0 1 0 0 0 1];Vb2=[1 0 0 0 1 0];%%整流级选择 26 15，为了将合成的电压矢量处于1扇区，此时的da必须大于等于db
...
elseif(sector==2&Uc_alphar_1>=0)%%为了保证直流环节为postive，可选择的整流级开关状态为（S2,S4),(S2,S6)以及(S2,S4),(S1,S6)
    ...
    Va2=[0 1 0 1 0 0];Vb2=[1 0 0 0 0 1];%%整流级选择24 16，为了将合成的电压矢量处于2扇区，此时的da必须大于等于db
    Vdc_a=Uc_B_1+(-1)*Uc_A_1;
    Vdc_b=Uc_A_1+(-1)*Uc_C_1;
...
elseif(sector==3)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S4),(S2,S4)以及(S3,S4),(S2,S6)
    sita=pi+sita;
    ...
    Va2=[0 0 1 1 0 0];Vb2=[0 1 0 0 0 1];%%整流级选择34 26，为了将合成的电压矢量处于3扇区，此时的da必须大于等于db
    Vdc_a=Uc_C_1+(-1)*Uc_A_1;
    Vdc_b=Uc_B_1+(-1)*Uc_C_1;
...
elseif(sector==4)%%为了保证直流环节为postive，可选择的整流级开关状态为（S3,S5),(S3,S4)以及(S3,S5),(S2,S4)
    sita=pi+sita;
    ...
    Va2=[0 0 1 0 1 0];Vb2=[0 1 0 1 0 0];%%整流级选择35 24，为了将合成的电压矢量处于4扇区，此时的da必须大于等于db
    Vdc_a=Uc_C_1+(-1)*Uc_B_1;
    Vdc_b=Uc_B_1+(-1)*Uc_A_1;
...
elseif(sector==5&Uc_alphar_1<=0)%%为了保证直流环节为postive，可选择的整流级开关状态为（S1,S5),(S3,S5)以及(S1,S5),(S3,S4)
    sita=pi+sita;
    ...
    Va2=[1 0 0 0 1 0];Vb2=[0 0 1 1 0 0];%%整流级选择15 34，为了将合成的电压矢量处于5扇区，此时的da必须大于等于db
    Vdc_a=Uc_A_1+(-1)*Uc_B_1;
    Vdc_b=Uc_C_1+(-1)*Uc_A_1;
...
elseif(sector==6)%%为了保证直流环节为postive，可选择的整流级开关状态为（S1,S6),(S1,S5)以及(S1,S6),(S3,S5)
    ...
    Va2=[1 0 0 0 0 1];Vb2=[0 0 1 0 1 0];%%整流级选择16 35，为了将合成的电压矢量处于6扇区，此时的da必须大于等于db 
    Vdc_a=Uc_A_1+(-1)*Uc_C_1;
    Vdc_b=Uc_C_1+(-1)*Uc_B_1;
    da2=sin(2*pi-sita);db2=sin(2*pi/3-(-sita));
    Vdc2=da2*Vdc_a+db2*Vdc_b;
    Ic_A_a=1*Idc;Ic_B_a=(0)*Idc;Ic_C_a=-1*Idc;%%%在整流级为100001时的Ic
    Ic_A_b=(0)*Idc;Ic_B_b=(-1)*Idc;Ic_C_b=(1)*Idc;%%%在整流级为001010时的Ic
    Ic_alphar_a=(2/3)* (Ic_A_a-(1/2)*Ic_B_a-(1/2)*Ic_C_a);      Ic_beta_a=(2/3)*((sqrt(3)/2)*Ic_B_a-(sqrt(3)/2)*Ic_C_a);
